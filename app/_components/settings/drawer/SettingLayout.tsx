...
import { setThemeLayout, setThemeStretch } from '@/app/_redux/slice/theme';
...
export default function SettingLayout() {
  const {themeLayout} = useSelector((state: RootState)=> state.theme)
  const dispatch = useDispatch()
  const handleChage = () => {
    let toggleBtn = themeLayout == 'horizontal' ? 'vertical' : 'horizontal'
    themeLayout !== 'vertical' &&  dispatch(setThemeStretch(true))
    dispatch(setThemeLayout(toggleBtn))
  }
  return (
    <RadioGroup name="themeLayout" value={themeLayout} onChange={handleChage}>
      <Grid dir="ltr" container spacing={2.5}>
        {['horizontal', 'vertical'].map((layout, index) => {
